...
def init_obj(opt, *args, default_file_name='default file', given_module=None, init_type='Network', **modify_kwargs):
    ...
    if opt is None or len(opt) < 1:
        print(f'Option is None when initialize {init_type}')
        return None
    ...
    try:
        ...
        if isinstance(attr, type):
            ...
            # ret = attr
        print(f'{init_type} [{class_name}() form {file_name}] is created.')
    except:
        raise NotImplementedError(f'{init_type} [{class_name}() form {file_name}] not recognized.')
    return ret
    ...
    for name in os.listdir('.'):
        if name in ['config', 'models', 'core', 'slurm', 'data']:
            shutil.copytree(name, os.path.join(
                opt['path']['code'], name), ignore=shutil.ignore_patterns("*.pyc", "__pycache__"))
