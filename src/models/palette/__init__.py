...
def create_model(model_opt):
    """ create_model """
    model = init_obj(
        model_opt, default_file_name='models.model', init_type='Model')
...
def define_network(network_opt):
    ...
    net = init_obj(
        network_opt, default_file_name='models.network', init_type='Network')

    if phase == 'train':
        print(
            f'Network [{net.__class__.__name__}] weights initialize using [{init_type}] method.')
        net.init_weights()
    return net
