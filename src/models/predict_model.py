...
inference_model_path: str                = config['models']['control_path']
if os.path.isfile(config['models']['last_path']):
    inference_model_path: str            = config['models']['last_path']

cldm_path: str                          = config['models']['cldm_path']
saving_memory: bool             = config['misc']['saving_memory']


model = create_model(cldm_path).cpu()
model.load_state_dict(load_state_dict(inference_model_path, location='cuda'))
model = model.cuda()
ddim_sampler = DDIMSampler(model)


def __resize_with_pad(image, desired_size):
...
def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
    with torch.no_grad():
        ...
        img = __resize_with_pad(input_image, image_resolution)
        H, W, C = img.shape
